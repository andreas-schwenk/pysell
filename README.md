...
- In general you may import arbitrary Python libraries. `pySELL` will do its best to map data types to internal data types (e.g. mostly used `sage` data types are mapped). For all unimplemented types, the variable is considered to be a _term_ and the value is exported by `str(my_var)`. This may work, or may not. Feel free to ask the author of `pySELL` to extend support of exotic data types.
...
In some cases, it may be beneficial to exclude the zero from random number generation. For example, a numerical question would be too easy to solve, if zero is drawn.
