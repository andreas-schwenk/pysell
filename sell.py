...
HTML += b'ument.createElement("h1");t.innerHTML=this.quizSrc.title,e.a'
HTML += b'ppendChild(t);let s=document.createElement("div");s.style.ma'
HTML += b'rginTop="15px",e.appendChild(s);let i=document.createElement'
HTML += b'("div");i.classList.add("pysell-author"),i.innerHTML=this.qu'
HTML += b'izSrc.author,e.appendChild(i);let o=document.createElement("'
HTML += b'p");if(o.classList.add("pysell-course-info"),e.appendChild(o'
HTML += b'),this.quizSrc.info.length>0)o.innerHTML=this.quizSrc.info;e'
HTML += b'lse{o.innerHTML=F[this.quizSrc.lang];let l=\'<span onclick="l'
HTML += b'ocation.reload()" style="text-decoration: none; font-weight:'
HTML += b' bold; cursor: pointer">\'+K[this.quizSrc.lang]+"</span>",h=d'
HTML += b'ocument.createElement("p");h.classList.add("pysell-course-in'
HTML += b'fo"),e.appendChild(h),h.innerHTML=O[this.quizSrc.lang].repla'
HTML += b'ce("*",l)}if(this.debug){let l=document.createElement("h1");'
HTML += b'l.classList.add("pysell-debug-code"),l.innerHTML="DEBUG VERS'
HTML += b'ION",e.appendChild(l)}}}generateQuestions(){let e=1;for(let '
HTML += b't of this.quizSrc.questions){let s=t.title;this.questionNumb'
HTML += b'ering&&(s=""+e+". "+s),t.title=s;let i=w();this.htmlElements'
HTML += b'.questions.appendChild(i);let o=new q(this,i,t,this.quizSrc.'
HTML += b'lang,this.debug);o.showSolution=this.debug,this.questions.pu'
HTML += b'sh(o),o.populateDom(this.timeLimited),this.debug&&t.error.le'
HTML += b'ngth==0&&o.hasCheckButton&&o.checkAndRepeatBtn.click(),e++}}'
HTML += b'runTimer(){let e=this.htmlElements.timerFooter;e.style.textA'
HTML += b'lign="center";let t=document.createElement("button");t.class'
HTML += b'List.add("pysell-button"),t.style.backgroundColor="var(--pys'
HTML += b'ell-green)",t.innerHTML=J[this.quizSrc.lang],t.addEventListe'
HTML += b'ner("click",()=>{this.timeLeft=1}),e.appendChild(t);let s=th'
HTML += b'is.htmlElements.timer;s.classList.add("pysell-timer"),s.inne'
HTML += b'rHTML=ce(this.timeLeft);let i=setInterval(()=>{this.timeLeft'
HTML += b'--,s.innerHTML=ce(this.timeLeft),this.timeLeft<=0&&this.stop'
HTML += b'Timer(i)},1e3)}stopTimer(e){let t=this.htmlElements.timerFoo'
HTML += b'ter;t.style.display="none",clearInterval(e);let s=0,i=0;for('
HTML += b'let h of this.questions){let n=h.src.points;i+=n,V(h),h.stat'
HTML += b'e===x.passed&&(s+=n),h.editingEnabled=!1}let o=this.htmlElem'
HTML += b'ents.timerEval;o.classList.add("pysell-eval");let l=document'
HTML += b'.createElement("h1");o.appendChild(l),l.innerHTML=i==0?"":""'
HTML += b'+s+" / "+i+" "+G[this.quizSrc.lang]+" <br/><br/>"+Math.round'
HTML += b'(s/i*100)+" %"}};function ce(r){let e=Math.floor(r/60),t=r%6'
HTML += b'0;return e+":"+(""+t).padStart(2,"0")}f(ce,"formatTime");fun'
HTML += b'ction be(r,e){let t={date:document.getElementById("date"),he'
HTML += b'ader:document.getElementById("header"),questions:document.ge'
HTML += b'tElementById("questions"),timer:document.getElementById("tim'
HTML += b'er"),timerInfo:document.getElementById("timer-info"),timerFo'
HTML += b'oter:document.getElementById("timer-footer"),timerEval:docum'
HTML += b'ent.getElementById("timer-eval")};new R(r,t,e),document.getE'
HTML += b'lementById("data-policy").innerHTML=$[r.lang]}f(be,"init");r'
HTML += b'eturn ge(ke);})();pysell.init(quizSrc,debug);</script></body'
HTML += b'> </html> '
